...
function OrdersContent() {
  ...
  return (
    <div className="space-y-6">
      <header className="mx-auto max-w-4xl flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Narudzbine</h1>
          <p className="text-sm text-slate-500">
            Prati narudzbine kroz faze (poruceno → poslato → stiglo → legle pare) i odmah racunaj svoj procenat
            profita.
          </p>
        </div>
      </header>
      ...
      <div className="mx-auto w-full max-w-4xl">
        ...
        <CardHeader>
          <CardTitle>{editingOrder ? "Izmeni narudzbinu" : "Nova narudzbina"}</CardTitle>
          {editingOrder && (
            <p className="text-sm text-slate-500">
                      ...
                      {isProductsLoading ? (
                        ...
                        filteredProducts.map((product, productIndex) => {
                          ...
                          return (
                            <div
                              key={product._id}
                              className={`border-b border-slate-100 last:border-b-0 ${productIndex % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}
                            >
                              <button
                                type="button"
                              ...
                              >
                                ...
                                {hasVariants && (
                                  <span className="text-[11px] font-semibold text-blue-600">{isExpanded ? "Zatvori" : "Tipovi"}</span>
                                )}
                              </button>
              ...
              <FormField
                ...
                render={({ field, fieldState }) => (
                  <FormItem>
                    ...
                    />
                    <p className="text-xs text-slate-500">
                      Rucno unesi trosak transporta u EUR (prihvata decimale).
                    </p>
                    <FormMessage>{fieldState.error?.message}</FormMessage>
                  </FormItem>
                )}
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
                      ...
                      <TableCell className="text-right font-semibold">
                        ...
                        <TableCell>
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <Button variant="outline" size="sm" onClick={() => handleStartOrderEdit(order)}>
                              Izmeni
                            </Button>
          ...
          <div className="space-y-4 md:hidden">
            {orderEntries.length === 0 ? (
              <p className="text-center text-sm text-slate-500">Jos nema narudzbina.</p>
            ) : (
              orderEntries.map(({ order, prodajnoUkupno, nabavnoUkupno, transport, prof, mojDeo }) => (
                <div key={order._id} className="space-y-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div className="space-y-1">
                      <p className="text-xs uppercase text-slate-400">{formatDate(order.kreiranoAt)}</p>
                      <StageBadge stage={order.stage} />
                      <h3 className="text-base font-semibold text-slate-800">{order.title}</h3>
                      {order.variantLabel && <p className="text-xs font-medium text-slate-500">{order.variantLabel}</p>}
                    </div>
                    <div className="text-right text-sm font-semibold">
                      ...
                      {order.myProfitPercent !== undefined ? (
                        <p className="text-xs text-slate-500">
                          Moj deo {formatCurrency(mojDeo, "EUR")} ({order.myProfitPercent}%)
                        </p>
                      ) : null}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-slate-700">{formatCurrency(transport, "EUR")}</p>
                      {order.transportMode ? (
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-600">{order.napomena || "-"}</p>
                    </div>
                  </div>
